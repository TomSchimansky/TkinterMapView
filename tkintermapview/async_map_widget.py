...
class AsyncTkinterMapView(tkinter.Frame):
    """
    Asyncrhonuous version of the TkinterMapView widget. Everything is single-
    threaded and uses async tasks for tile fetchings instead.
    """
    running: bool
    """Running flag"""
    bg_color: str
    """Background color in hex format `#xxxxxx`"""
    width: int
    """Widget width in pixels"""
    height: int
    """Height width in pixels"""
    corner_radius: int
    """Widget rounded corners radius"""
    canvas: tkinter.Canvas
    button_zoom_in: CanvasButton
    """Zoom in button"""
    button_zoom_out: CanvasButton
    """Zoom out button"""
    last_mouse_down_position: Optional[Tuple]
    """Last mouse down position coordinates (if any)"""
    last_mouse_down_time: Optional[float]
    """Last mouse down position time (if any)"""
    mouse_click_position: Optional[Tuple]
    """Last mouse click position coordiantes (if any)"""
    map_click_callback: Optional[Callable]
    ...
    tile_image_cache: Dict[str, PIL.ImageTk.PhotoImage]
    ...
    def __init__(self, *args,
        ...
        if bg_color is None:
            ...
            if (hasattr(self.master, "canvas") and hasattr(self.master, "fg_color")) or (hasattr(self.master, "_canvas") and hasattr(self.master, "_fg_color")):
                ...
                # customtkinter version <=4.6.3
                    ...
                    if isinstance(self.master.fg_color, tuple) or isinstance(self.master.fg_color, list):
                        self.bg_color: str = self.master.fg_color[self.master._appearance_mode] #type: ignore
                    else:
        ...
        # zoom buttons
        self.button_zoom_in = CanvasButton(self, (20, 20), text="+", command=self.button_zoom_in_callback) #type: ignore
        self.button_zoom_out = CanvasButton(self, (20, 60), text="-", command=self.button_zoom_out_callback) #type: ignore
    ...
    def set_position(self, deg_x, deg_y, text=None, marker=False, **kwargs) -> Optional[CanvasPositionMarker]:
    ...
    def set_address(self, address_string: str, marker: bool = False, text: Optional[str] = None, **kwargs) -> Union[CanvasPositionMarker, bool, None]:
        ...
        if result.ok:
            ...
            if hasattr(result, "bbox"):
                ...
                for zoom in range(self.min_zoom, self.max_zoom + 1):
                    lower_left_corner = decimal_to_osm(*result.bbox['southwest'], zoom) #type: ignore
                    upper_right_corner = decimal_to_osm(*result.bbox['northeast'], zoom) #type: ignore
            ...
            return self.set_position(*result.latlng, marker=marker, text=text, **kwargs) #type: ignore
        else:
            return False

    def set_marker(self, deg_x: float, deg_y: float, text: Optional[str] = None, **kwargs) -> CanvasPositionMarker:
        marker = CanvasPositionMarker(self, (deg_x, deg_y), text=text, **kwargs) #type: ignore
        marker.draw()
        self.canvas_marker_list.append(marker)
        return marker

    def set_path(self, position_list: list, **kwargs) -> CanvasPath:
        path = CanvasPath(self, position_list, **kwargs) #type: ignore
        path.draw()
        self.canvas_path_list.append(path)
        return path

    def set_polygon(self, position_list: list, **kwargs) -> CanvasPolygon:
        polygon = CanvasPolygon(self, position_list, **kwargs) #type: ignore
        polygon.draw()
            ...
            if last_pre_cache_position is not None and radius <= 8:
                ...
                for x in range(self.pre_cache_position[0] - radius, self.pre_cache_position[0] + radius + 1):
                    if f"{zoom}{x}{self.pre_cache_position[1] + radius}" not in self.tile_image_cache:
                        await self.request_image(zoom, x, self.pre_cache_position[1] + radius, db_cursor=db_cursor)
                    if f"{zoom}{x}{self.pre_cache_position[1] - radius}" not in self.tile_image_cache:
                        await self.request_image(zoom, x, self.pre_cache_position[1] - radius, db_cursor=db_cursor)
                ...
                for y in range(self.pre_cache_position[1] - radius, self.pre_cache_position[1] + radius + 1):
                    if f"{zoom}{self.pre_cache_position[0] + radius}{y}" not in self.tile_image_cache:
                        await self.request_image(zoom, self.pre_cache_position[0] + radius, y, db_cursor=db_cursor)
                    if f"{zoom}{self.pre_cache_position[0] - radius}{y}" not in self.tile_image_cache:
                        await self.request_image(zoom, self.pre_cache_position[0] - radius, y, db_cursor=db_cursor)
    ...
    async def request_image(self, zoom: int, x: int, y: int, db_cursor=None) -> ImageTk.PhotoImage:
            ...
            if self.overlay_tile_server is not None:
                ...
                async with aiohttp.ClientSession() as session:
                        async with session.get(url, headers=headers) as response:
                            response.raise_for_status()  # Raise an error for HTTP status codes >= 400
                            raw_data = await response.read()  # Read the raw response data
                            image_overlay = Image.open(BytesIO(raw_data))  # Create a PIL image from the raw data
        ...
        # except grequests.exceptions.ConnectionError:
        #     return self.empty_tile_image
    ...
    def insert_row(self, insert: int, y_name_position: int):
        ...
        for x_pos in range(len(self.canvas_tile_array)):
            ...
            if image is False:
                canvas_tile = CanvasTile(self, self.not_loaded_tile_image, tile_name_position) #type: ignore
    ...
    def set_zoom(self, zoom: int, relative_pointer_x: float = 0.5, relative_pointer_y: float = 0.5):
        ...
        current_tile_mouse_position = decimal_to_osm(*current_deg_mouse_position, round(self.zoom)) #type: ignore
